...
class AssuranceGestController extends Controller
{
    ...
    public function index()
    {
        ...
        if($verifRole->isAdmin() OR $verifRole->isAssurance()){
            return response()->json([
                'status' => 'success',
                'data' => User::select('id_user','nom','ville','quartier','tel','email','id_assurance','photo_profil')->where('role', Roles::Assurance->value)->with('assurance:id_assurance,libelle')->get(),
                'message' => 'gestionnaires assurance récupérés avec succès.'
            ],200);
        }else{
            return response()->json([
                'status' => 'error',
